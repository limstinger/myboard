...
## total_result view 추가 --------------------------------------------------
@app.route('/fms_result')
def fms_result():
    conn = get_db_connection()
    cursor = conn.cursor(cursor_factory=DictCursor)

    # 1) 원본 테이블(표) - 도착일 기준 정렬 추천
    cursor.execute('SELECT * FROM fms.total_result ORDER BY "도착일" DESC NULLS LAST LIMIT 200')
    results = cursor.fetchall()

    # 2) ✅ KPI: "부적합여부"로 합격/불합격 계산
    cursor.execute("""
        SELECT
            COUNT(*)::int AS total_cnt,
            SUM(CASE WHEN "부적합여부" IN ('Y','y','1','부적합') THEN 0 ELSE 1 END)::int AS pass_cnt,
            SUM(CASE WHEN "부적합여부" IN ('Y','y','1','부적합') THEN 1 ELSE 0 END)::int AS fail_cnt
    ...
    cursor.close()
    conn.close()
    return render_template('fms_result.html', results=results, kpi=kpi)


# 
@app.route('/api/fms/pass_fail')
def api_fms_pass_fail():
    conn = get_db_connection()
    ...
    cur.execute("""
        SELECT
            CASE
                WHEN "부적합여부" IN ('Y','y','1','부적합') THEN 'F'
                ELSE 'P'
            END AS pass_fail,
            COUNT(*)::int AS cnt
        FROM fms.total_result
...
@app.route('/api/fms/top_farms')
def api_fms_top_farms():
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)
...
@app.route('/api/fms/arrival_trend')
def api_fms_arrival_trend():
    conn = get_db_connection()
    cur = conn.cursor(cursor_factory=RealDictCursor)

    cur.execute("""
        SELECT
            "도착일"::date AS arrival_date,
            COUNT(*)::int AS cnt
        FROM fms.total_result
        GROUP BY "도착일"
        ORDER BY "도착일"
